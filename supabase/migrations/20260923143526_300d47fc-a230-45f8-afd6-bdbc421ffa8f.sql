...
CREATE TABLE public.support_requests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_phone text NOT NULL DEFAULT '',
  message text NOT NULL DEFAULT 'Cliente solicitou atendimento humano',
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.whatsapp_sessions TO anon, authenticated;
...
CREATE POLICY "support_requests_public_all" ON public.support_requests FOR ALL TO anon, authenticated USING (true) WITH CHECK (true);
CREATE POLICY "whatsapp_sessions_public_all" ON public.whatsapp_sessions FOR ALL TO anon, authenticated USING (true) WITH CHECK (true);
