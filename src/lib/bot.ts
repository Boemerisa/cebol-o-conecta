  ...
  switch (state.step) {
    case "human": {
      if (n === "voltar" || n === "reiniciar" || n === "menu") {
        return {
          state: initialBotState(),
          replies: [{ text: "Voltamos ao in�cio! Como posso te ajudar hoje?", buttons: WELCOME_BUTTONS }],
        };
      }
      return {
        state,
        replies: [
          { text: "Sua mensagem foi recebida e ser� respondida em breve. ?? Se quiser voltar ao menu principal, digite 'voltar'." },
        ],
        action: "human",
      };
    }
